...
11. **무효 내부 드롭 폴스루(최종 리뷰 I2)**: 폴더를 끌어 제 행 위·자기 하위 행에서 놓기→무동작(루트로 이동 금지, 하이라이트도 안 뜸). 트리 폴더 행에서 특히 확인(폴스루 시 조용히 루트로 이동했던 결함).
12. **stale 스냅샷 회귀(최종 리뷰 C1)**: 앱→Finder로 폴더를 드래그 아웃(스냅샷 잔존)한 뒤, Finder→앱으로 같은 폴더 행에 파일 드롭→정상 수락(외부 세션이 stale draggingURLs를 읽어 죽은 타깃이 되지 않는지).
```
