...
> 원본의 마크다운 리더 위에 **이미지·PDF 보기**, **한글/오피스(HWP·DOCX·XLSX) 읽기**, **HWP/HWPX 편집 후 서식 보존 저장**, **종류를 가로지르는 내용 검색**, **Claude 연동(`claude -p`)**을 더한 개인용 도구입니다.
...
swift run                           # run it
swift test                          # 125 tests (정식 Xcode 필요)
...
`Process`로 호출합니다. 외부 도구는 비샌드박스 환경에서 서브프로세스로 실행됩니다.
