...
- Phase 8.5(②b·③ 남음) — PARA 라이브러리 뷰. "파일 하나 읽는 리더" 위에 "폴더를 펼쳐 훑는 라이브러리"를 더하고, PARA를 출력(라우팅)뿐 아니라 **탐색 축**으로도 쓴다(`legoSeed` 재사용). (1) `AppState.mainMode`(reader/library) 추가, `MainEditorView`가 분기, 툴바에 리더⇄라이브러리 세그먼트(하위 보조토글은 모드 따라 Source·Split·Preview ⇄ List·Grid 교체). (2) 클릭 동선 절충 — 파일 클릭→reader, 폴더 클릭→library 자동전환하되 토글이 우선(덮어쓰기), 관통축은 "현재 폴더". (3) 폴더 **선택** 개념 신설 — 현재 트리는 펼치기만 있고 선택이 없음. `selectedFolder`(폴더 탭 시 set) 도입, 펼치기(disclosure)와 별개. (4) 라이브러리 뷰 = detail에 `LazyVGrid`/`List`, 항목은 `selectedFolder` 직속 파일+하위폴더, 그리드셀=썸네일+이름+종류, 파일 클릭→리더로 줌인. (5) 썸네일 = `QLThumbnailGenerator`(QuickLook)로 이미지·PDF·영상·오피스 전 타입, lazy 생성+`NSCache`, 화면 밖 생성 보류(1,000+ 대비). (6) PARA 렌즈 = 사이드바 트리 루트를 `legoSeed` 기준 정렬/그룹, 경로로 분류 판별(`40000_Archive` 하위 dim·`10000_Projects` 하위 또렷). (7) 폴더별 뷰 기억 = `[URL: LibraryLayout]`(list/grid)을 `SessionState`/설정에 저장·복원. 안 만드는 것: 편집·동기화·모바일·신규 영상플레이어(썸네일까지만). **읽기/탐색 전용 — 파일 이동·삭제 없음**(정리는 Phase 8 몫). 단계: ①사이드바 PARA 렌즈(가벼움·즉시 PARA 느낌) → ②메인 라이브러리 그리드+모드토글 → ③폴더별 뷰 기억·다듬기. 비고: Phase 8(폴더정리)과 "폴더 한눈에 보기" 화면을 공유 — 8.5 그리드가 8 정리의 무대가 되도록 둘을 잇는다.
- Phase 8.7 (계획) — 미리보기 속도 다듬기. 기능을 쌓는 동안 미뤄둔 프리뷰 성능 병목을 제거한다(코드 검증으로 확인된 것만).
  1. **외부 자산 로컬 번들화(최우선).** 현재 `MarkdownRenderer.swift`의 `hljsIncludes`(highlight.js)·`wrapWithHTML`(KaTeX·Mermaid)이 매 렌더마다 `cdn.jsdelivr.net`에서 로드 → 네트워크 대기·오프라인 취약. 세 라이브러리의 JS/CSS를 앱 번들 리소스로 포함하고 file:// 또는 WebView 주입으로 로드. 버전 고정 효과.
     - **THIRD-PARTY-NOTICES.md 갱신 필수**: 번들로 전환되면 Mermaid·KaTeX·highlight.js가 §2(CDN 로드·미포함)에서 §1(바이너리 포함)로 이동, 각 MIT 전문 동봉. ※현재 §2에 highlight.js가 누락된 것으로 보임 — 이번에 함께 바로잡기(확인 필요).
  2. **파일 트리 백그라운드화.** `AppState.buildFileTree`가 `FileManager.contentsOfDirectory`를 메인 스레드에서 재귀 실행(depth 10) → 큰 폴더 열기·펼치기 시 멈춤. 스캔을 `Task.detached`로 옮기고 결과만 메인 반영.
  3. **(검토) 미리보기 부분 갱신.** 타이핑 중 WKWebView 통째 리로드 대신 본문(DOM)만 교체+디바운스. PreviewView 렌더 호출 방식을 코드로 확인한 뒤 확정(추정 금지).
  - 규율: 작업 전후 `swift test`(208개) 통과 확인. 신규 분은 별도 파일/모듈로 분리해 upstream 머지 용이.
  - 위치: 8.5(②③) 다음, 9(시맨틱+RAG) 앞.
- 다음 액션: 티어 3 — Phase 8 폴더정리(배치, Claude 제안→승인→FileManager 이동·로그·undo) → 8.5 PARA 라이브러리 뷰 → 9 시맨틱+RAG → 10 다듬기·배포.
