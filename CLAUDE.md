...
- GitHub 공개 저장소 연결: `origin` = https://github.com/learn-slowly/cmd-docu (Public). `upstream` = 원작자 CmdMD. PR #1(cmd-docu→main) 열림.
  - 주의: `gh pr` 등은 `upstream` 원격 때문에 베이스를 원작자 저장소로 오인할 수 있음 → `--repo learn-slowly/cmd-docu` 명시 필요.
- 다음 액션: Phase 2 — PDF 리더(PDFKit, `PDFView` 래핑·페이지/썸네일/검색/선택·복사/줌/회전).
